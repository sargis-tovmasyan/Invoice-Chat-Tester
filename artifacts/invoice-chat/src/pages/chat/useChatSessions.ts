...
export function useChatSessions() {
  ...
  const renameSession = useCallback((sessionId: string, title: string) => {
    ...
    setSessions((current) => current.map((session) => session.id === sessionId ? { ...session, title: nextTitle, updatedAt: now() } : session));
  }, []);

  const archiveSession = useCallback((sessionId: string) => {
    setSessions((current) => {
      const next = current.map((session) => session.id === sessionId ? { ...session, archived: true, updatedAt: now() } : session);
  ...
  const clearActiveSession = useCallback(() => {
    setSessions((current) => current.map((session) => session.id === activeSessionId ? { ...session, messages: [], updatedAt: now() } : session));
  }, [activeSessionId]);

  const toggleRaw = useCallback((messageId: string) => {
    setSessions((current) => current.map((session) => session.id === activeSessionId ? {
      ...session,
      messages: session.messages.map((message) => message.id === messageId ? { ...message, showRaw: !message.showRaw } : message),
    } : session));
  }, [activeSessionId]);
